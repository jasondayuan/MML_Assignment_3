...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    x = x.transpose(1, 0, 2)
    h = np.zeros((T, h0.shape[0], h0.shape[1]))
    ...
    for t in range(T):
        if t == 0:
            ...
            h_prev = h[t - 1]
        # h[t] is h_t+1
        h[t], n_cache = rnn_step_forward(x[t], h_prev, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dh_cur = dh[t] + dh_prev
        dx_t, dh_prev, dWx_t, dWh_t, db_t = rnn_step_backward(dh_cur, cache[t])
        dx[t] += dx_t
        dh0 = dh_prev
        dWx += dWx_t
        dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, next_c, next_h, tmp)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, next_c, next_h, tmp = cache
    N, H = dnext_h.shape
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        if t == 0:
            ...
            h_prev = h[t - 1]
        h[t], c, n_cache = lstm_step_forward(x[:, t], h_prev, c, Wx, Wh, b)
        cache.append(n_cache)
...
def lstm_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dh_cur = dh[t] + dh0
        dx_t, dh0, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh_cur, dc, cache[t])
        dx[t] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
