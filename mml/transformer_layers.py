...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        t = torch.arange(0, max_len)[:, None]
        omega = torch.exp(torch.arange(0, embed_dim, 2) * - (torch.log(torch.tensor(10000.0)) / embed_dim))
        pe[0, :, 0::2] = torch.sin(omega * t)
        pe[0, :, 1::2] = torch.cos(omega * t)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        output = (attn_weights @ value).transpose(1, 2).contiguous()
