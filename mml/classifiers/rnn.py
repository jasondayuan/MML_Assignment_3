...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
          ...
          x, cache_embed = word_embedding_forward(captions_in, W_embed)
          cache_lstm = None
          h_out, cache_lstm = lstm_forward(x, h0, Wx, Wh, b)
        
          y, cache_vocab = temporal_affine_forward(h_out, W_vocab, b_vocab)
          loss, dy = temporal_softmax_loss(y, captions_out, mask)

          dh_out, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dy, cache_vocab)
          dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh_out, cache_lstm)
          grads['W_embed'] = word_embedding_backward(dx, cache_embed)
          _, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, cache_proj)
    ...
    def sample(self, features, max_length=30):
        ...
        word = (self._start * np.ones((N, 1))).astype(int)
        ...
        if self.cell_type == 'rnn':
          for t in range(max_length): 
            ...
            h, _ = rnn_step_forward(word_embed.reshape(N, D), prev_h, Wx, Wh, b)

            scores, _ = temporal_affine_forward(h.reshape(N, 1, H), W_vocab, b_vocab)

            index = np.argmax(scores.reshape(N, V), axis=1)

            captions[:, t] = index
            word = captions[:, t].reshape(N, 1)
          ...
          for t in range(max_length): 
            ...
            h, prev_c, _ = lstm_step_forward(word_embed.reshape(N, D), prev_h, prev_c, Wx, Wh, b)
